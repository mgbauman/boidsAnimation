...
void Camera::panLeftRight(float distance)
{

    vec3 newPos = vec3(distance,distance, distance) * right; 
    pos += newPos;
}

//Not working as intended
void Camera::panUpDown(float distance)
{
    vec3 newPos = vec3(distance, distance, distance) * up;
    pos += newPos;
}
